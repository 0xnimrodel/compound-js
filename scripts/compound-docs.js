...
import { parse } from 'docblock-parser';
...
function getAllFilePaths(srcPath, resultPaths) {
  dirOrFileNames = readdirSync(srcPath);

  resultPaths = resultPaths || [];

  dirOrFileNames.forEach((name) => {
    if (statSync(srcPath + name).isDirectory()) {
...
srcFilePaths.forEach((f) => {
  const fileContents = readFileSync(f);
  const blocks = getDocBlocks(fileContents);

  if (blocks.length > 0) {
    blocks.forEach((b) => {
      const parsed = unSlashBug(parse(b.contents));
      mdResult += markdownify(parsed, b.functionName);
    });
  }
});

writeFileSync(outFilePath, mdResult);
